...
enum LocalProviders {
    ...
    static func transcribe(_ url: URL) async throws -> String {
        ...
        guard await AssetInventory.status(forModules: [transcriber]) == .installed else { throw VoiceError.missing }
    ...
    static func reply(to transcript: String) async throws -> String {
        guard SystemLanguageModel.default.availability == .available else { throw VoiceError.missing }
