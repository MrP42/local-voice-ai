# Settings

Every setting has a tab. What you choose while working, such as model or voice, is not here but in the footer or on the page itself.

## Dictation

Shortcuts, push-to-talk or toggle, voice activity detection, custom words, paste method and clipboard behaviour. Plus the transcription language and the model for meetings.

## Read aloud

Volume, export format and options for installed read-aloud modules. Manage additional Piper voices and their runtime under **Models → Reading voices**. Choose the voice while reading aloud.

<!-- module:fish -->
The installed Fish module also provides folder, port, idle stop, prewarming and voice management.
<!-- /module:fish -->

## Microphone & sounds

Input device, output device, level, start and stop sounds, mute while recording.

## AI text improvement

AI text improvement uses your selected language model. Under Models, install a local model directly or choose an available system model. External providers are optional; manage their connections, prompts and budgets here. Usage per model is shown in the overview.

## General

Account & devices for syncing between computers, interface language, theme, autostart, tray, overlay, retention, data directory, updates.

## About

Version, licences, log files for a bug report.
